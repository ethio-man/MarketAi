          ...
          <div className="w-full max-w-md bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden flex flex-col">
            ...
            <div className="flex items-center justify-between p-4 border-b border-gray-100 bg-white">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <p className="font-semibold text-gray-800 text-sm">{brand || "Your Brand"}</p>
                  <p className="text-xs text-gray-500">Sponsored</p>
                </div>
              </div>
              <div className="flex space-x-3">
                <button onClick={() => navigator.clipboard?.writeText(caption || "")} className="text-blue-500 text-sm font-semibold hover:text-blue-700 transition" title="Copy Caption">
                  Copy
                </button>
                <button onClick={sendMessage} className="text-blue-500 text-sm font-semibold hover:text-blue-700 transition" title="Regenerate All">
            ...
            <div className="relative w-full aspect-square bg-gray-50 flex items-center justify-center border-b border-gray-100">
              {imageError && (
                <div className="absolute inset-0 flex items-center justify-center bg-white/95 z-20">
                  <p className="text-sm text-red-600 font-medium px-6 text-center">{imageError}</p>
                </div>
              )}
              {isGeneratingImage && (
              ...
              {imageUrl ? (
                <>
                  {imageLoading && !isGeneratingImage && (
                    <div className="absolute inset-0 bg-gray-100/70 flex flex-col items-center justify-center text-blue-500 z-10 backdrop-blur-sm">
                      <div className="animate-spin rounded-full h-10 w-10 border-b-2 border-blue-600 mb-3"></div>
                      <p className="font-medium text-sm drop-shadow-sm">Loading from server...</p>
                    </div>
                  )}
                  <img
                    src={imageUrl}
                    ...
                    onError={() => { setImageLoading(false); setImageError("Image failed to load. Please try again."); setImageUrl(""); }}
                  />
                </>
              ) : !isGeneratingImage && (
                <div className="w-full h-full flex flex-col items-center justify-center text-gray-400 p-8 text-center bg-gray-100">
                   ...
                   <p className="text-sm">Your generated ad image will appear here</p>
                </div>
              )}
            </div>
            ...
            <div className="p-4 pb-3 bg-white">
              <div className="flex justify-between items-center mb-3 text-gray-800">
                <div className="flex space-x-4">
                  <svg className="w-6 h-6 hover:text-red-500 hover:fill-red-500 cursor-pointer transition" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path></svg>
                  <svg className="w-6 h-6 hover:text-blue-500 cursor-pointer transition" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path></svg>
                  <svg className="w-6 h-6 hover:text-blue-500 cursor-pointer transition" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"></path></svg>
                </div>
                <svg className="w-6 h-6 hover:text-gray-500 cursor-pointer transition" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z"></path></svg>
              </div>

              {/* Likes */}
              <p className="font-semibold text-sm text-gray-900 mb-2">1,245 likes</p>

              {/* Caption */}
              <div className="text-sm text-gray-800 leading-relaxed">
                <span className="font-bold mr-2 text-gray-900">{brand || "Your Brand"}</span>
                {caption ? (
                  <span>{caption}</span>
                ) : (
                  <span className="text-gray-500 italic">Generate a campaign to see the polished AI caption here...</span>
                )}
              </div>
              ...
              <p className="text-xs text-gray-400 mt-3 uppercase tracking-wide">Sponsored • Just now</p>
            </div>
          </div>
        </div>
      </div>
